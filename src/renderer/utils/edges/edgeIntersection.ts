...
export interface Point {
   ...
   * Falls back to the center if the geometry is degenerate.
   *
   * @param from   - Origin of the ray (typically the source node center)
   * @param target - The rectangle to intersect (typically the destination node)
   */

  export const intersectRectBorder = (from: Point, target: Rect): Point => {
    const { cx, cy, width, height } = target;
  ...
  export const getBorderPoints = (
    ...
    const sourceCenter: Point = { x: source.cx, y: source.cy };
    const targetCenter: Point = { x: target.cx, y: target.cy };
